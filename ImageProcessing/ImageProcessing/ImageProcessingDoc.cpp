...
BEGIN_MESSAGE_MAP(CImageProcessingDoc, CDocument)
	...
	ON_COMMAND(ID_BILNEAR, &CImageProcessingDoc::OnBilnear)
...
{
	//메시지 큐에 있는 메시지들을 다음과 같이 직접 처리해 주는 방법도 있다.
	//이렇게 메시지처리 함수를 하나 만들어 놓고,
	//시간이 걸리는 함수 중간 중간에 위 함수를 호출해 주면
	//함수 실행 도중이더라도 사용자가 누른 버튼이나 키보드 메시지가 정상적으로 처리된다.
	MSG msg;
	...
	if (dlg.DoModal() == IDOK) { // DoModal 대화상자의 활성화 여부
		...
		for (i = 0; i < m_Re_height; i++) { //가장 가까운 픽셀의 값을 참조해서 보간하는 방법
			for (j = 0; j < m_Re_width; j++) {
				...
				rx = double((m_width - 1) * i / (m_Re_width - 1));
				ry = double((m_height - 1) * j / (m_Re_height - 1));
...
void CImageProcessingDoc::OnBilnear()
{
